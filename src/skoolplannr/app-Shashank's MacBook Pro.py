...
class PesuAttendancePayload(BaseModel):
    username: str
    password: str
...
def sign_up(payload: AuthPayload) -> Dict:
    try:
        ...
        result = auth.sign_up(payload.email, payload.password, payload.name)
        ...
        raise HTTPException(status_code=status.HTTP_502_BAD_GATEWAY, detail="AUTH_SERVICE_UNAVAILABLE") from exc
...
@app.post("/attendance/pesu")
async def fetch_pesu_attendance(
    payload: PesuAttendancePayload,
    x_user_id: Optional[str] = Header(default=None),
) -> Dict:
    _required_uid(x_user_id)
    service = PesuService()
    try:
